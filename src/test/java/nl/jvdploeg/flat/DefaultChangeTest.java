...
public class DefaultChangeTest {
    ...
    @Before
    public void before() {
        changeOne = new DefaultChange(ChangeAction.ADD, new Path(new String[] { "A", "B" }), "old", "new");
        changeEqual = new DefaultChange(ChangeAction.ADD, new Path(new String[] { "A", "B" }), "old", "new");
        changeDifferentAction = new DefaultChange(ChangeAction.SET, new Path(new String[] { "A", "B" }), "old", "new");
        changeDifferentPath = new DefaultChange(ChangeAction.ADD, new Path(new String[] { "A", "OTHER" }), "old", "new");
        changeDifferentOldValue = new DefaultChange(ChangeAction.ADD, new Path(new String[] { "A", "B" }), "other", "new");
        changeDifferentNewValue = new DefaultChange(ChangeAction.ADD, new Path(new String[] { "A", "B" }), "old", "other");
    }
    ...
    public void testHashcode() {
        ...
        // different hashcodes
        Assert.assertEquals(5, new HashSet<Change>(
                Arrays.asList(changeOne, changeEqual, changeDifferentAction, changeDifferentPath, changeDifferentOldValue, changeDifferentNewValue))
                        .size());
    }
