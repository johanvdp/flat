...
public class JsonRoundTripTest {
    ...
    public void testChangeRoundTrip() throws ParseException {
        final Change original = new DefaultChange(ChangeAction.SET, new Path(new String[] { "A", "B", "C" }), "old", "new");
    ...
    public void testResponseRoundTrip() throws ParseException {
        final Message message1 = new DefaultMessage("id1", Severity.INFO, "two () ()", new String[] { "A", "B" });
        final Message message2 = new DefaultMessage("id2", Severity.ERROR, "one ()", new String[] { "A" });
        final Response original = new DefaultResponse("id0", Arrays.asList(message1, message2));
        final String json = JsonEncoder.encode(original).toString();
        final Object object = JsonDecoder.decode(json);
