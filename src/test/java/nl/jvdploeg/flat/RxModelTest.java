...
public class RxModelTest {
    ...
    public void testColdObservable() {
        ...
        Assert.assertEquals(DefaultChange.add(new Path(new String[] { "A" })), consumer.getCollection().get(0));
        Assert.assertEquals(DefaultChange.set(new Path(new String[] { "A" }), "a"), consumer.getCollection().get(1));
        Assert.assertEquals(DefaultChange.add(new Path(new String[] { "A", "B" })), consumer.getCollection().get(2));
        Assert.assertEquals(DefaultChange.set(new Path(new String[] { "A", "B" }), "b"), consumer.getCollection().get(3));
    }
