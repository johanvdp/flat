...
public abstract class NodeUtils {
    ...
    /**
     * Follow the path and add (create) the last node in the path. Creating only the last node when {@link Enforcement#STRICT} or all nodes when
     * {@link Enforcement#LENIENT}.
     *
     * @param node
     *            The node.
     * @param path
     ...
     *            The enforcement mode.
     */
    private static Node addLastAt(final Node node, final Path path, final int index, final Enforcement enforcement) {
    ...
    private static void removeLastAt(final Node node, final Path path, final int index) {
        ...
        if (index == path.getLength() - 1) {
            ...
            if (child == null) {
                final String error = String.format("%s can not be removed because %s (%d) does not exist", path, name, index);
                LOG.error(error);
